...
//было для рабочего режимаparameter dlinaImit=361;
...
input goslave; // вторая матрица еще читает кадр
wire goslave;
wire sumgo; // обе матрицы еще читаются
...
output outgo; 
reg outgo; //сигнал на запуск другой матрицы который пойдет в slave
...
input ready;// разрешение фиксирования выстрела внутри запущенной процедуры кадра
wire ready;
...
reg [30:0] temptimer; // временная перемнная
...
// clockADC сдвинут относительно clock матрицы на 2 такта=20 нс
				...
				if(faza==6)begin adcreg=acdata; cnreg<=cndata; linereg<=linecount; end// защелкнули данные в регистре
//отрабатывается циклограмма матрицы все изменения делаются в faza=0
		if(faza==0 )begin
			...
			// формирование синхро сигнала запуска slave матрицы в slave rksgo=ingo(outgo) !!!! как только posedge outgo то firstslave <=1; одиочный импульс за одни запуск
...
//     первые 25 clock                         от заднего фронта fsync
	if((0<=kodclocka && kodclocka<=50) && (kadrvi==1) && fixstartkadr==0)
		begin intakadr<=1; end //выстрел попал в фазу до начала ресета можно сразу читать кадр
...
// закончили читать кадр делаем запрет на повторный цикл
	if(kodclocka==((line+2)*(lineclk+16)*2+200+1))begin resetgo<=1; dlinatime<=dlinay; end //считали кадр и остановились до следующего запуска
		else resetgo<=0;
...
or (sumgo,go,goslave); // обе матрицы еще чтото делают оба сигнала должны быть 0 - тогда кадры готовы
// для кадров
...
// разрешение сигнала синхронизации на slave
always @(posedge bls0we or posedge outgo) //firstslave устанавливается в 1 при запуске кадра и засекается в 0 по заднему фронту когда сгенерился outgo
	begin
...
always @(*)
	...
	begin
	mreset<=~0;//поскольку инверсия то 0 - на матрицу придет 0 (LOW BWL) жестко равно LOW BW
	mfield<=~0;//поскольку инверсия то 0 - на матрицу придет 0 (HIGH GAIN)надо добавит в регистр для управления сейчас большой коэф усиления
	end
...
assign svetodiod=outgo; // мигаем светодиодом 
